 ...
 * no per-request state — no `setStatus`/`setHeader` accumulator like tsoa's
 * base class, which would race between concurrent requests. To vary the
 * status code, use `@SuccessResponse` for the happy path and throw an
 * `ApiError` otherwise; to write a response directly, take `@Res()`.
 */
