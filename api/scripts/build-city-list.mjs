 ...
 *
 * This was originally written against Nominatim, OpenStreetMap's geocoder, and
 * that was the wrong tool: Nominatim is an interactive search service whose
 * usage policy asks for no more than one request a second, which makes four
 * hundred lookups a seven-minute crawl that gets your address rate-limited if
 * anything else is talking to it at the same time. A gazetteer download is
 * both the polite option and the reproducible one — the same input gives the
 * same output, offline, for ever.
 *
